...
注意：上述构建产物是旧包路径，图标和最新 UI 已在源码中接入；如需安装包体现最新图标和 UI，需要重新运行 `npm.cmd run dist`。

2026-05-19 已重新运行 `npm.cmd run dist`，当前 `release/` 产物已包含最新 UI、正式图标、首次启动向导、目录快捷按钮、`127.0.0.1` 桌面加载修复和 AI Coding 专属预设。

注意：2026-05-20、2026-05-21 和 2026-05-23 的批量任务、视觉分析、规则库、历史销量导入和插件质量报告等源码能力已经完成，但如需安装包包含最新能力，需要再次运行 `npm.cmd run dist`。
